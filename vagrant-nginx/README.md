...
/etc/securettyファイルへrootユーザーがログインできる端末を指定する。  
下記記述でsecurettyファイルを空にすることでログインを禁止する。

    $ su root
    $ echo > /etc/securetty

__EC2はでフォルトでrootログインを禁止している。__  
__デフォルトのログインユーザーははubuntu。__
...
\<user\>,\<password\>は登録したユーザーとパスワードを記載する。
